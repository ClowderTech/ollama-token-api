...
function flattenTools(
	...
	const walk = (tool: unknown, ns: string | null, depth: number) => {
		...
		// 4. everything else (local_shell, file_search, mcp, tool_search…) stays unsupported,
		//    but is logged so the drop is never silent.
...
function reverseIndex(alias: Map<string, Alias>) {
	...
	const byName = new Map<string, string | null>(); // null => ambiguous across namespaces
...
/** input[].function_call history items the client already sent must use wire names too. */
...
/** A tool_choice pointing at a dropped/flattened tool must not 400 upstream. */
...
/** Inverse of flatten: mcp__srv__tool -> { name:"tool", namespace:"mcp__srv__" }. */
...
/** Codex wants output_tokens_details. On Responses streams usage nests under `response`. */
...
// Hop-by-hop / framing headers that must not be copied verbatim between hops.
	...
	async (c: Context) => {
		try {
			...
			if (isGetLike) {
				...
				const text = await c.req.text(); // string in, string out — no typed arrays
				try {
					...
					if (parsed && Array.isArray(parsed.tools)) {
						...
						upstreamBody = text; // /v1/responses with no tools[] -> verbatim
					}
				} catch {
					upstreamBody = text; // not JSON we can help with -> verbatim
				}
			} else {
				upstreamBody = c.req.raw.body; // stream untouched (images, multipart, big uploads)
			}
			...
			reqHeaders.set("accept-encoding", "identity"); // keep the body readable for rewriting
			...
			if (contentType.includes("text/event-stream")) {
				...
				const ts = new TransformStream({
					transform(chunk, controller) {
						...
						buffer = lines.pop() ?? ""; // keep a possibly-split tail (handles \r\n too)
